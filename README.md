...
To speed up computation, if multiple GPUs are available, several instances of `compute_stats.py` can be launched, and computation can be parallelized in the batch dimension. 

It is recommended to launch one instance of `compute_stats.py` for each dataset/model/dataset_split/checkpoint, and to further process batches of paths in parallel across several independent compute jobs.

Our code supports two ways of parallelizing computation across several GPU workers:
1. If a single instance of `compute_stats.py` is run with multiple GPUs, then one GPU worker is instantiated for each GPU automatically. Each worker will process batches of `BATCH_SIZE` paths.
2. Alternatively (recommended), one instance of `compute_stats.py` can be lauched for each available GPU. Then, `--l_skip-paths` can be used to split the available paths across GPU workers. For instance, for 2 GPU workers, processing `256` paths and using batch size `128`:
```bash
# GPU 0
  --l_num-paths=256 --batch-size=128
# GPU 1
  --l_num-paths=256 --batch-size=128 --l_skip-paths=128
```
will assign the first batch of 128 paths to rank 0 and the others to rank 1.

**Note**: the code performs several internal optimizations in order to discover linear regions and compute absolute deviation efficiently. For data-driven paths with `a` anchor points, and batch size of `b` paths, one forward pass of effective batch size `B = b * a * 2` is performed for each linear region.
